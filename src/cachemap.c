...
static PyMethodDef CacheMap_methods[] = {
    {"evict", (PyCFunction) CacheMap_evict, METH_NOARGS, "evict a item. raise error if no item in cache raise error if no item in cache."},
    ...
    {"setnx", (PyCFunction) CacheMap_setnx, METH_VARARGS | METH_KEYWORDS, "like setdefault but accept a callable object which takes no args"},
